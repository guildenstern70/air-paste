...
import { getPaste, recordUsage, savePaste } from "./db.ts";

export class AirPasteRouter {
  ...
  public handle = async (req: Request): Promise<Response> => {
    ...
    if (url.pathname === "/css/style.css") {
      try {
        const cssPath = join(this.staticDir, "css/style.css");
    ...
    if (url.pathname === "/js/script.js") {
      try {
        const jsPath = join(this.staticDir, "js/script.js");
    ...
    if (url.pathname.startsWith("/img/")) {
      try {
        const imgPath = join(this.staticDir, url.pathname);
